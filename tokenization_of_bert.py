...
import numpy as np
import torch
from transformers import BertTokenizer, BertModel,  RobertaModel, RobertaTokenizer,GPT2Model,GPT2Config,GPT2ForSequenceClassification
...
def tokenization_for_codebert(data, max_length, flag,model_type):
    tokenizer_codeBERT = RobertaTokenizer.from_pretrained("./CodeBERT")
    tokenizer_BERT=RobertaTokenizer.from_pretrained("./pre-trainedBERT")

    if flag == 'msg':
        features = convert_examples_to_features(examples = data, max_seq_length=max_length, tokenizer=tokenizer_BERT)
        # else:
        #     features = convert_examples_to_features(examples=data,
        #                                             max_seq_length=max_length,
        #                                             tokenizer=tokenizer_codeBERT)
        return features
    elif flag == 'code':
        if model_type=="BERT":
            features =  convert_examples_to_hierarchical_features(examples = data, max_seq_length=max_length,tokenizer=tokenizer_BERT)
        else:
            features = convert_examples_to_hierarchical_features(examples=data,
                                                                 max_seq_length=max_length,
                                                                 tokenizer=tokenizer_codeBERT)
